...
getPostR postId = do
    ...
    comments <- runDB $ selectList [CommentPost ==. postId] [Asc CommentDate]
...
postCommentNewR :: PostId -> Handler Html
postCommentNewR postId = do
    name <- lookupPostParam "comment_name"
    text <- lookupPostParam "comment_text"
